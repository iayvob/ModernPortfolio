...
export default function Header() {
  ...
  return (
    <header className="z-[999] relative">
      <motion.div
        className={clsx(
          "fixed top-6 left-1/2 -translate-x-1/2 h-[3.25rem] rounded-full border border-white border-opacity-40 bg-white bg-opacity-80 shadow-lg shadow-black/[0.03] backdrop-blur-[0.5rem] w-[29rem] items-center dark:bg-gray-950 dark:border-black/40 dark:bg-opacity-75",
          {
            "!w-[5rem]":
          ...
          {links.map((link) => (
            ...
            >
              ...
              >
                <p
                  className={clsx({
                    "text-[#946263]": link.name === "Services",
                  })}
                >
                  {" "}
                  {link.name}
                </p>
                {link.name === "Services" && <FiExternalLink />}
