...
export default function ArticleList() {
  ...
  return (
    <div ref={ref} id='allArticles' className='sm:max-w-[60%] max-w-[100%] mt-10 mb-10'>
        ...
        <ul className="flex flex-col">
                {
                    article.map((article, index) => (
                        <motion.li className="flex flex-col p-3 mb-5 border dark:border-slate-800 hover:dark:border-white border-gray-200 hover:border-black duration-300 rounded-2xl" key={index} initial={{ y: -100, opacity: 0 }} animate={{ y: 0, opacity: 1 }}>
                            <Link className="italic font-bold mb-3 text-lg rounded-full hover:bg-gray-200 hover:text-black text-gray-700 dark:hover:bg-gray-800 dark:hover:text-white hover:underline p-2 dark:text-gray-300 duration-300" href={article.link} onClick={() => { }}>
                                👉 {article.title}
                            </Link>
